...
# Embedded Signup / coexistence. Both are public identifiers (the app
# id and a Login-for-Business configuration id), never secrets — the
# secret half of that flow is META_APP_SECRET, which stays runtime-only.
# Unset is a valid state: the button simply renders disabled.

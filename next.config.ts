...
const nextConfig: NextConfig = {
  /**
   * Standalone build output, for the Docker image.
   *
   * Emits `.next/standalone` containing a minimal `server.js` plus only
   * the traced `node_modules` files the server actually needs, so the
   * runtime image can skip `npm ci` entirely. `public/` and
   * `.next/static/` are NOT copied automatically — the Dockerfile does
   * that explicitly after the build (see the bundled docs at
   * `node_modules/next/dist/docs/01-app/03-api-reference/05-config/01-next-config-js/output.md`).
   *
   * Harmless for `next dev` / `next start`; it only adds a build artifact.
   */
  output: "standalone",
